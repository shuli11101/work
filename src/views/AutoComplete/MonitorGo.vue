...
const handleNext = () => {
  router.push('/auto-complete/done?from=monitorGo')
}
...
<template>
  <StepLayout :active-step="1" left-top-title="" left-bottom-title="" right-title="冲刺方案概览" title="冲刺模拟"
    second-title="模拟真实考试场景，针对重点难点进行专项冲刺，提升应试能力" @prev="$router.back()" @next="handleNext">
    ...
    <template #leftBottomBody>
      <div class="bottom-title-row">
        <span class="header-vertical-bar"></span>
        <span class="bottom-title">冲刺知识体系结构</span>
      </div>
